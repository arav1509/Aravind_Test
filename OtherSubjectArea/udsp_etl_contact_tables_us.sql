...
CREATE OR REPLACE PROCEDURE `rax-abo-72-dev`.slicehost.udsp_etl_contact_tables_us()
begin

----1:06
INSERT INTO
	`rax-abo-72-dev`.slicehost.act_val_accountstatus
select
	ID, status AS Name, 'Uknonwn' AS Description, 0 AS Online
from 
	`rax-landing-qa`.cms_ods.account_statuses
WHERE
    upper(status_source_system_name)='HMDB_US'    
AND ID NOT in (select id from `rax-abo-72-dev`.slicehost.act_val_accountstatus);
---------------------------------------------------------------------------------------


---------------------------------------------------------------------------------------
call `rax-abo-72-dev`.slicehost.udsp_etl_cloud_account_contact_info_current();

call `rax-abo-72-dev`.slicehost.udsp_etl_cloud_account_contact_info_active_current();

call `rax-abo-72-dev`.slicehost.udsp_etl_cloud_closed_accounts();

call `rax-abo-72-dev`.slicehost.udsp_etl_cloud_account_log_status_changes();


DELETE FROM `rax-abo-72-dev`.slicehost.act_log_accountstatus
WHERE
...
INSERT INTO	`rax-abo-72-dev`.slicehost.act_log_accountstatus
SELECT  
	     Date
...
AND	UPDATEDDATE >=  date_trunc(current_date(),year)
...
WHERE ACT_val_AccountStatusID is not null;
