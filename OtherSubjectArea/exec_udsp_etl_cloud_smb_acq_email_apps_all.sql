CREATE OR REPLACE PROCEDURE `rax-abo-72-dev`.sales.exec_udsp_etl_cloud_smb_acq_email_apps_all()
begin
...
SET PriorMonthWorkDays = ( SELECT PriorMonthWorkDays 
							...
							WHERE upper(Reporting_Group_Type)= 'BILL FILE' AND Reporting_Month_Key=`rax-abo-72-dev`.bq_functions.udf_yearmonth_nohyphen(CurrentMonth)
							); 
...
IF  CalDays <= PriorMonthWorkDays then --EXECUTE ENT ACQ for prior month unit 3rd business day of current month
	...
	call `rax-abo-72-dev`.sales.udsp_etl_cloud_smb_acq_email_apps_all(PriorMonth);

ELSEIF CalDays >= CurrentMonthWorkDays then --EXECUTE ENT ACQ for current month after 7 business day  of current month

	call `rax-abo-72-dev`.sales.udsp_etl_cloud_smb_acq_email_apps_all(CurrentMonth);
