create or replace procedure `rax-abo-72-dev`.sales.exec_udsp_etl_email_apps_service_line_item()
begin
...
IF  	CalDays <= PriorMonthWorkDays then --EXECUTE ENT ACQ for prior month unit 3rd business day of current month
	...
	call `rax-abo-72-dev`.sales.udsp_etl_email_apps_service_line_item(PriorMonth);
ELSEIF CalDays >= CurrentMonthWorkDays then --EXECUTE ENT ACQ for current month after 7 business day  of current month

	call `rax-abo-72-dev`.sales.udsp_etl_email_apps_service_line_item(CurrentMonth);
end if;
end;
