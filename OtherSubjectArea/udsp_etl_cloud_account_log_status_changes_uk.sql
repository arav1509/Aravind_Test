CREATE OR REPLACE PROCEDURE `rax-abo-72-dev`.cloud_uk.udsp_etl_cloud_account_log_status_changes()
begin

create or replace temp table data_Temp as
SELECT ----into	data_Temp
...
-------------------------------------------------------------------------------------------------------------------
 ...
 create or replace table `rax-abo-72-dev`.cloud_uk.cloud_account_log_status_changes as
SELECT 
	ACT_AccountID, 
...
FROM 
	data_Temp A
INNER JOIN
	`rax-abo-72-dev`.cloud_uk.act_val_accountstatus B
