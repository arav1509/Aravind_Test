create or replace procedure `rax-abo-72-dev`.sales.exec_udsp_etl_cloud_smb_ib_acq_email_apps_invoice_audit()
begin
...
IF  	CalDays <= PriorMonthWorkDays then --EXECUTE ENT ACQ for prior month unit 3rd business day of current month
	...
	call `rax-abo-72-dev`.sales.udsp_etl_cloud_smb_ib_acq_email_apps_invoice_audit(PriorMonth);
ELSEIF CalDays >= CurrentMonthWorkDays then --EXECUTE ENT ACQ for current month after 7 business day  of current month

	call `rax-abo-72-dev`.sales.udsp_etl_cloud_smb_ib_acq_email_apps_invoice_audit(CurrentMonth);
end if;
end;
