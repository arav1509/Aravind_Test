...
dag = DAG(
    dag_id="NRD_GDO_Utility_Booking",
...
################ - Load_Utility_Bookings_Attributes - ################
Load_Utility_Bookings_Attributes_Start = DummyOperator(task_id="NRD_GDO_Utility_Booking_Start")
Load_Utility_Bookings_Attributes_End = DummyOperator(task_id="NRD_GDO_Utility_Booking_End")

previous_task = ''
stage_task = []
for task_id in JOB_ARGS['Load_Utility_Bookings_Attributes']:
    finished = DummyOperator(task_id="{}_completed".format(task_id))
    if previous_task:
        Load_Utility_Bookings_Attributes_Start = previous_task
    print(task_id)
    for procs in JOB_ARGS['Load_Utility_Bookings_Attributes'][task_id]:
        ...
        if (len(stage_task) == 1):
            Load_Utility_Bookings_Attributes_Start >> stage_task >> finished
        elif (len(stage_task) > 1):
            Load_Utility_Bookings_Attributes_Start >> stage_task >> finished
        stage_task.clear()
    previous_task = finished

finished >> Load_Utility_Bookings_Attributes_End
